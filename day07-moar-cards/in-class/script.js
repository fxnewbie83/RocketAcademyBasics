...
// The player can input how many cards they want.
// The lowest of the player's cards is compared against the computer's 1 card.
// Show the player all the cards that they drew.
// Rules from above exercises continue to apply.
...
var moarCardsLowCardHandsMain = function (input) {
  ...
  var playerCard = []; //[playerCardOne, playerCardTwo, playerCardThree];
  for (var i = 1; i <= input; i++) {
    var onePlayerCard = shuffledDeck.pop();
    playerCard.push(onePlayerCard);
  }
  //mininum
  var minCardRank = 100;
  for (var i = 1; i < playerCard.length; i++) {
    if (playerCard[i].rank < minRank) {
      minCardRank = playerCard[i].rank;
      minCardName = playerCard[i].name;
      minCardSuit = playerCard[i].suit;
    }
  }
